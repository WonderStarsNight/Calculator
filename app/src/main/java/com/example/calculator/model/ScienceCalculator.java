...
public class ScienceCalculator {
    ...
    private final BaseCalculator baseCalculator = new BaseCalculator();

    public double cal(String math, int precision, boolean angle_metric) {
        ...
        while (math.contains("^")) {
            ...
            if(result==Double.MAX_VALUE||result==Double.NEGATIVE_INFINITY||result==Double.POSITIVE_INFINITY){return Double.MAX_VALUE;};
        ...
        while (math.contains("!")){
            //1.找到阶乘符号
            int lastIndex = math.lastIndexOf("!");

            //2.获取左边参数
            double leftNum=-1; //左边的数
            String leftStr; //左边math字符串
            ...
            if (math.charAt(leftIndex) == ')') {        //1.左边是一个表达式，即左边用括号括起来
                ...
                leftStr = math.substring(leftIndex + 1, lastIndex);
                try {
                    leftNum = Integer.parseInt(leftStr);
            ...
            int result=1;
            for (int i=2;i<=leftNum;i++){
                result*=i;
            }
            math=math.replace(leftStr+"!",String.valueOf(result));
        }
        //处理E-
        for(int i=0;i<math.length();i++){
            if(math.charAt(i)=='E'&&math.charAt(i+1)=='-'){
                int firstIndex=i;
                i++;
                while (math.charAt(++i)!=')');
                math=math.substring(0,firstIndex+1)+"("+math.substring(firstIndex+1,i)+")"+math.substring(i);
            }
        }

        //(5)此时的math已经替换到BaseCalculator可处理的形式
        double result=baseCalculator.cal(math);
